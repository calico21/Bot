...
# --- FIXED: Lowered RSI default to 30 for safety ---
...
class MarketState:
    ...
    def __init__(self, prices, date, market_ticker='SPY', bond_ticker='IEF'):
        ...
        hist_vol_window = hist_vol_window.dropna().tail(756) 
...
class FortressSubStrategy:
    ...
    def get_hrp_weights(self, returns_df):
        ...
        try:
            ...
            dist = np.sqrt((1 - corr).clip(0, 2) / 2)
    ...
    def get_attack_portfolio(self, prices: pd.DataFrame, date: pd.Timestamp, regime: str):
        ...
        for ticker in valid_cols:
            ...
            # --- FIXED: HARD TREND FILTER (Stop buying crashing assets) ---
            sma_200 = series.rolling(200).mean().iloc[-1]
            current_price = series.iloc[-1]
            if current_price < sma_200:
                continue # Skip asset if it's in a long-term downtrend
...
def is_rebalance_day(api_key, secret_key, paper=True):
    ...
    try:
        ...
        if today not in trading_days: return False
        if len(trading_days) < 2: return True
