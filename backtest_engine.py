...
def run_full_strategy_backtest(strat_override=None):
    ...
    for date in monthly_dates:
        ...
        for t in all_tickers:
            gross_turnover += abs(target_dict.get(t, 0.0) - current_portfolio.get(t, 0.0))
...
# ============================
# 3. Institutional Grade Plotting (Saves to 'reports/' folder)
# ============================
...
def plot_advanced_dashboard(base_equity):
    ...
    ax1.set_title("Underwater Plot", fontweight='bold')
...
def plot_monte_carlo(equity_curve, n_sims=100000):
    ...
    plt.plot(equity_curve.index[1:], p50, color='black', linestyle='--', label='Median')
    plt.plot(equity_curve.index, equity_curve, color='#d62728', linewidth=2, label='Actual')
    plt.yscale('log')
    plt.title(f"Monte Carlo Stress Test", fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
...
def plot_yearly_efficiency(equity_curve):
    ...
    y = yearly_rets.loc[common] * 100
    x = yearly_vol.loc[common] * 100
    
    plt.figure(figsize=(10, 8))
    plt.scatter(x, y, c=y, cmap='RdYlGn', s=150, edgecolors='black')
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, "chart_5_efficiency.png"), dpi=300)
    plt.close()
...
def plot_regime_timeline(prices, strat, equity_curve):
    ...
    plt.title("Regime Timeline", fontweight='bold')
    plt.tight_layout()
